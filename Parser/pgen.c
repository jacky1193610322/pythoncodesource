...
compile_item(labellist *ll, nfa *nf, node *n, int *pa, int *pb)
{
    ...
    if (n->n_type == LSQB) {
        ...
        *pb = addnfastate(nf);
        ...
        if (n->n_type == STAR)
            /* 这边将pb设置为pa了 跟之前的连接一条pa－》pb empty的策略不对， 而是将pb的起点设置为了pa  
             * 也就是说本来从pb出发的边 现在完全可以从pa出发  也就是省了这条empty的转换  你也许会想 如果后面没有要连接 pb后面的 那怎么半
             * 看一下compile_rhs 一开始传进来的就是 start 和 finish 那么此时的finish的结束正好是 开始的那个state 这个跟我们连一条线
             * 是一样的结果 这种方式可能是为了避免环路的形成  但是现在还没有看看见其真正的用意  但是最终的结果是一样的*/
            *pb = *pa;
        else
            REQ(n, PLUS);
    }
...
makedfa(nfagrammar *gr, nfa *nf, dfa *d)
{
    ...
    /* 一开始就是只有一个 那就是上面从起始状态开始出发经过empty找到的状态集合 */
    for (istate = 0; istate < xx_nstates; ++istate) {
        ...
        for (ibit = 0; ibit < nf->nf_nstates; ++ibit) {
            if (!testbit(ss, ibit))
                continue;
            st = &nf->nf_state[ibit];
            /* For all non-empty arcs from this state... 从closure里面的状态出现找到不是经过empty的边 
            也就是相当于找第二个closure集合*/
            for (iarc = 0; iarc < st->st_narcs; iarc++) {
                ar = &st->st_arc[iarc];
                /* 为什么上面已经找到了这个状态的closure 还会有empty 因为这个状态出发到closure里面的状态 */
                if (ar->ar_label == EMPTY)
                    continue;
                /* Look up in list of arcs from this state 
                查看经过这个边的是不是在之前已经出现了 那么合并他们就行了 比如1 ————> 2 经过a 
                1 ————> 3 经过a 那么在1到2的过程中已经创建了相应的arc  */
                for (jarc = 0; jarc < yy->ss_narcs; ++jarc) {
                    zz = &yy->ss_arc[jarc];
                    if (ar->ar_label == zz->sa_label)
                        goto found;
                }
                /* 如果上面没有找到已经创建的 那么就新创建一个arc 也就相当于新的边 */
        ...
        /* 上面的创建只是创建边 他的bitset集合也是在arc里面暂存的 但是并没有创建新的状态 
        一定注意我说的边 是将原来存在empty的边去掉这个过程 */
