...
OPT= -DNDEBUG -g -fwrapv -Wall -Wstrict-prototypes
BASECFLAGS=	 -fno-strict-aliasing
...
$(PYTHONFRAMEWORKDIR)/Versions/$(VERSION)/$(PYTHONFRAMEWORK): \
	...
	if test "${UNIVERSALSDK}"; then \
		...
		/usr/bin/libtool -o $(LDLIBRARY) -dynamic $(OTHER_LIBTOOL_OPT) $(LIBRARY) \
			 -lSystem -lSystemStubs -arch_only x86_64 -install_name $(PYTHONFRAMEWORKINSTALLDIR)/Versions/$(VERSION)/$(PYTHONFRAMEWORK) -compatibility_version $(VERSION) -current_version $(VERSION) ;\
	fi
	$(INSTALL) -d -m $(DIRMODE)  \
